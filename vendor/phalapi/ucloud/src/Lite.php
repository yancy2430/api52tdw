...
class Lite
{
     ...
     */
    public function upfile($fileData, $base64 = false)
    {

        //处理base64上传
        if ($base64) {

            $base64_image_content = trim($fileData);
            //正则匹配出图片的格式
            if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)) {
                ...
                $file['savename'] = $this->file_name . $type;
                $file['data'] = str_replace($result[1],"",$fileData);
                if ($upload->base64ToImage($file)) {
                    $fileName = $this->setPath() . '/' . $this->file_name . $type;
